...
module.exports = {
  ...
  getDaysBetween(
    ...
    if (moment(startDate, format) === moment(endDate, format))
      return [moment(startDate, format).format(formatStr)];

    let days = this.getCountBetween(startDate, endDate);
    let rs = [];
    for (let i = 0; i <= days; i++) {
      rs.push(moment(startDate, format).add(i, 'd').format(formatStr));
    }
    return rs;
  },
  ...
  */
  ...
  formatDurationString(
    ...
    if (msg.length === 0) msg = `0 ${lang.second}`;
    return msg;
  },
