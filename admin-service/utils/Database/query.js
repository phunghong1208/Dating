...
module.exports = {
      ...
      for (let filter of inputs) {
        ...
        if (filter.columnName) {
          ...
          switch (dataType) {
            ...
            default:
              console.log(
                `unknown data type: ${dataType} in GridTable's filter.`,
              );
          }
          ...
          switch (operation) {
            case 'contains':
              condition = new RegExp(
                this.escapeRegExp(String(value).trim()),
                'i',
              ); // không phân biệt hoa thường. search LIKE
              break;
            case 'notContains':
              condition = {
            ...
            case 'daterange':
              ...
              condition = {
                $gte: new Date(DateUtil.getStartOfDate(value.startDate, true)),
                $lte: new Date(DateUtil.getEndOftDate(value.endDate, true)),
              };
            ...
            default:
              console.log(
                `unknown operation: ${operation} in GridTable's filter.`,
              );
          }
          if (condition) {
            ...
            let arr = key.split('.');
            let path = arr.shift();
            if (Utils.inArray(relations, path) && arr.length) {
              key = arr.join('.');
  ...
  setFilters(options, filters = [], relations = []) {
    if (filters.length) {
      let rs = this.buildFilterForGridTable(filters, relations);
  ...
  // loại bỏ những bản ghi xoá mềm
  excludeSoftDelete(query) {
    if (!query) query = {};
    if (!query.delete) query.delete = { $exists: false };
    return query;
  },

  excludeSoftBlock(query) {
    if (!query) query = {};
    if (!query.block) query.block = { $exists: false };
    return query;
  },
};
